...
dataset <-read.csv("~/churn/old/data/rawdata/churn.csv") %>% 
...
dataset1 = dataset %>%
  ...
  select(-c(contains('deposit_vs_withdrawal')))
...
base_rec <- 
  ...
  step_pca(all_numeric_predictors(),num_comp = 10) %>%prep()
  
train_sc <- base_rec %>%  bake(new_data = train) %>% 
  mutate(Churn = as.factor(Churn))
test_sc  <- base_rec %>%  bake(new_data = test)%>%
  mutate(Churn = as.factor(Churn))
# train_sc

# upsample minority class

train_sc_ <- upSample(train_sc,y = train_sc$Churn) %>% select(-Class) %>% 
  mutate(Churn = ifelse(Churn == 'No',1,0))
```
...
logit.CV <- train(x = train_sc_ %>% select(-Churn) , y= as.factor(train_sc_$Churn), 
                  method = 'glm',trControl = trControl,family = 'binomial' )
