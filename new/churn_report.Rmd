...
## Pre variable_selection selection

```{r}
dataset1 = dataset %>% select(-c(client_id,minWithdrawal,age,HasFixedDeposit)) 
# remove high corelated variables

library(caret)

hc <- findCorrelation(x = corr_stats,cutoff = .90,names = T)

dataset1 <- dataset1 %>% 
  select(-hc) %>% 
  bind_cols(dataset %>% select(HasFixedDeposit)) %>%
  mutate(Churn = ifelse(HasFixedDeposit == 1 & Churn == "Yes","No",Churn))

splits <- initial_split(data = dataset1,strata = 'Churn',prop = .8)
train <- training(splits)
test <- testing(splits)

train %>% ExpCTable(Target = "Churn",margin = 2) 
...
base_rec <- 
  recipe(Churn ~ .,
         data=train %>% mutate(
           ...
           ProductsSub = as.factor(ProductsSub),
         ) %>% 
  step_dummy(all_nominal_predictors())


base_rec %>% prep() %>% bake(new_data = train)
```
