...
```{r}
dataset1 = dataset %>% select(-c(minWithdrawal,age,HasFixedDeposit)) 
```
...
tidymodels_prefer()
set.seed(1)
churn_sc_data <- initial_split(dataset1,prop = .8,strata = "Churn")

train <- training(churn_sc_data)
test <- testing(churn_sc_data)

set.seed(1)
bird_folds <- vfold_cv(train, strata = Churn)

glm_spec <- logistic_reg()

base_mod <- recipe(Churn~.,data = train) %>% 
  update_role(client_id,new_role = "sample ID") %>% 
  step_dummy(all_nominal_predictors()) %>% 
  step_center(na_rm = T)
  
  
wf_basic <- workflow(base_mod,glm_spec) 
